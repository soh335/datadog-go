...
* If the `addr` parameter is empty, the client uses the `DD_DOGSTATSD_URL` and then `DD_AGENT_HOST` environment
  variables to build a target address. `DD_DOGSTATSD_URL` takes precedence over `DD_AGENT_HOST`, but both use the same
  format.
...
* `DD_DOGSTATSD_PORT` can be use to set the UDP port. If `DD_DOGSTATSD_URL` or `DD_AGENT_HOST` already contains a port
